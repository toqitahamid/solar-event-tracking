...
def helioviewer(csv_file, event_type, wavelength, df):

        data = pd.read_csv(csv_file, usecols=['event_starttime', 'event_endtime', 'hpc_bbox', 'event_type'])
            ...
            while start <= end:
                ...
                try:
                    ...
                    if (prev_filename != filename): 
                        ...
                        if (check_if_exist(filename)):
                           
                            jp2_location = get_jp2_file_location(filename)
                           ...
                           #     copyfile(jpg_location_dump, jpg_location_dst)
                            ...
                            #print(jp2_location)
                            print(start)
                            #jp2dump_glob = glob.glob('jp2dump/CH/193/*.jp2')
                            filename_not_match = re.search(r'\d{4}_\d{2}_\d{2}__\d{2}_\d{2}_\d{2}_\d{2}__SDO_AIA_AIA_171.jp2', jp2_location)
                            print(filename)
                            if filename_not_match is None:
                                print("true")
                                jp2_info = download_jp2(start, directoryJp2, event_type)
                                print('jp2_info')
                                print(jp2_info)
                                jp2_info_match = re.search(r'\d{4}_\d{2}_\d{2}__\d{2}_\d{2}_\d{2}_\d{2}__SDO_AIA_AIA_171', jp2_info)
                                
                                jp2_info_match_filename = jp2_info_match.group(0)
                                print('jp2_info_match_filename')
                                print(jp2_info_match_filename)
                                jp2_location = directoryJp2 + jp2_info_match_filename + '.jp2'    
                            ...
                            if (filename == jp2_filename):
                                ...
                                if (os.stat(jp2_location).st_size > 702500):
...
def create_annotation(annotation_counter, image_counter, jp2_location, bbox_x, bbox_y, first_match, directoryAnnotation):
    if (annotation_counter == 0):
        ...
        save_pixel_coordinate(image_counter, bbox_pixel_x, bbox_pixel_y, directoryAnnotation, annotation_filename)
...
def get_jp2_file_location(filename):    
    #names = [os.path.basename(x) for x in glob.glob('test/*.jp2')]
    
    for x in glob.glob('jp2dump/AR/171/*.jp2'):
...
if __name__ == "__main__":
    ...
    parent_directory = sorted(glob.glob('../data_collection/test/' + event_type + '/*.csv'), key=numericalSort)
    
    df = pd.read_csv('filenames/image_names_ar_12_17.csv', usecols=['event_starttime', 'jp2_filename'])
    image_counter = 0
    for csv_file in parent_directory:
        helioviewer(csv_file, event_type, wavelength, df)
        #print(csv_file)
